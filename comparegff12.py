...
parser = argparse.ArgumentParser(
     prog='comparegff12.py',
     usage='''python comparegff12.py --gff [Combined gff file] --classified [classified fasta file] --repbase [Repbase library file] --path [Path to input files]''',
...
with open("known_" + filename1,'w') as xyz:
 with open("unknown_" + filename1,'w') as pqr:
  for line in open("all_" + filename1,'r'):
    i+=1
    if (alignments[i] >= 80) and (percentidentity[i] >= 80):  # Invoking the 80/80 rule
        xyz.write("%s" %(line)) 
    else:
        pqr.write("%s" %(line))
        unknowns+=1   
...
with open(filename1[:len(filename1)-8] + "_final8080.gff",'w') as xyz:
